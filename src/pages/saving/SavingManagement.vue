  ...
  <div class="container-fluid bg-light-green min-vh-100 d-flex flex-column">
    ...
    <div class="text-center my-3">
      <button class="btn" @click="goToAddSaving">
        <img src="@/assets/images/saving/add_saving.png" alt="새로운 저축 등록하기" class="img-fluid rounded-3 shadow-sm" />
      </button>
    </div>
    ...
    <div class="flex-grow-1 px-3 overflow-auto d-flex flex-column bg-white rounded-4">
      ...
      <div v-else class="list-box d-flex flex-column gap-3 mb-5">
        <div v-for="saving in savingStore.savings" :key="saving.id" class="list-innerbox p-3 bg-white shadow-sm" @click="router.push(`/saving/${saving.id}`)">
          ...
          <div class="custom-light d-flex justify-content-between" style="font-size: 0.8rem">
            <div>
              ₩{{ (saving.saved || 0).toLocaleString() }}<br /><small class="text-start opacity-75">적립</small>
            </div>
            <div>
              {{ saving.percent }}%<br /><small class="d-block text-center opacity-75">진행</small>
            </div>
            <div>
              ₩{{ (saving.targetAmount || 0).toLocaleString() }}<br /><small class="d-block text-end opacity-75">목표</small>
            </div>
          </div>
        </div>
      </div>
